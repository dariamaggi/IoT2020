...
RESOURCE(act_temperature,
         "title=\"Cooling system controller: ?POST/PUT state=ON|OFF\";obs;rt=\"Temperature Sensor\"",
         res_get_handler,
...
static void res_get_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
  if(request != NULL) 
	...
	if(accept == APPLICATION_XML) {
		...
		snprintf((char *)buffer, COAP_MAX_CHUNK_SIZE, "<state=\"%d\"/>", cooling_active);
...
static void res_post_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
	...
	if((len = coap_get_post_variable(request, "state", &state))) {
		if(strncmp(state, "ON", len) == 0) {
			cooling_active = 1;
			LOG_DBG("Cooling Started! \n");
			leds_set(LEDS_NUM_TO_MASK(LEDS_GREEN));
		} else if(strncmp(state, "OFF", len) == 0) {
			cooling_active = 0;
			LOG_DBG("Cooling Stopped \n");
